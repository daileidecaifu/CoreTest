...
/**
 * 数据SQLite操作关联实现 可查阅 <a href="http://www.sqlite.org/lang.html">SQLite操作指南</a>
 *
 * @author MaTianyu
 * @date 2015-03-13
 */
public final class CascadeSQLiteImpl extends LiteOrm {
  ...
  @Override
  public int update(final Object entity, final ColumnsValue cvs,
  ...
  public <T> int delete(Class<T> claxx, WhereBuilder where) {
    ...
    try {
      ...
      List<T> list = query(
          QueryBuilder.create(claxx).columns(new String[]{table.key.column}).where(where));
      delete(list);
    } catch (Exception e) {
      e.printStackTrace();
    } finally {
   ...
   * 过程：
   * <p/>
   * 1. 根据条件查找符合条件的所有当前对象 2. 遍历所有当前对象，遍历其所有关联对象，读取关系表 map:<key1, key2> 3.
   * 如果是多对一，根据map查找key2的关联对象，赋给obj1 4. 如果是一对多，根据map查找key2的关联对象，反射实例化obj1的容器，关联对象放入。 5. 并对关联对象递归此过程
   */
   ...
   * 循环遍历查找当前实体的关联实体
   */
  private void queryForMappingRecursive(Object obj1, SQLiteDatabase db,
   ...
   * @return return size of collection if do successfully, -1 or not.
   */
  private <T> int insertCollection(final Collection<T> collection,
      final ConflictAlgorithm conflictAlgorithm) {
    if (!Checker.isEmpty(collection)) {
      SQLiteDatabase db = mHelper.getWritableDatabase();
      Integer rowID = Transaction.execute(db, new Worker<Integer>() {
   ...
   * @return rowID of entity
   */
  private long checkTableAndSaveRecursive(Object obj1, SQLiteDatabase db,
        ...
        if (map.isToOne()) {
          ...
          if (array != null) {
            if (array instanceof Collection<?>) {
              ...
              // 一定要强转为(Object[])
              handleMapToMany(table1, key1, Arrays.asList((Object[]) array), db, insertNew,
                  handleMap);
            } else {
              throw new RuntimeException("OneToMany and ManyToMany Relation, " +
                  "you must use collection or array object");
